%% ========================================================================
%  FedPDA η_g Sweep 비교 분석 — Dirichlet α=0.1 (Severe Non-IID)
%  ========================================================================
...
%% ========================= 데이터 로드 ==================================
merged = readtable(fullfile(DATA_DIR, 'merged_accuracy_a0.1.csv'), 'VariableNamingRule','preserve');
staleness = readtable(fullfile(DATA_DIR, 'merged_staleness_a0.1.csv'), 'VariableNamingRule','preserve');
...
title('FedPDA: Accuracy vs. Round (\alpha=0.1, \eta_g sweep)');
...
title('FedPDA: Accuracy vs. Simulation Time (\alpha=0.1, \eta_g sweep)');
legend('Location','southeast'); ylim([0 85]);
...
%% ========================================================================
fprintf('[Fig 3] Loss vs Round (alpha=0.1)\n');
figure('Position',[100 100 900 500]); hold on; grid on; box on;
...
title('FedPDA: Loss vs. Round (\alpha=0.1, \eta_g sweep)');
...
%% ========================================================================
fprintf('[Fig 4] Staleness Over Time (alpha=0.1)\n');
figure('Position',[100 100 900 500]); hold on; grid on; box on;
...
title('FedPDA: Staleness Over Time (\alpha=0.1, \eta_g sweep)');
...
%% ========================================================================
fprintf('[Fig 5] Plane Contributions (alpha=0.1)\n');
...
plane_files = {
    fullfile(DATA_DIR, 'a0.1_eta0.1_plane_contributions.csv')
    fullfile(DATA_DIR, 'a0.1_eta0.3_plane_contributions.csv')
    fullfile(DATA_DIR, 'a0.1_eta0.5_plane_contributions.csv')
};
...
title('FedPDA: Per-Plane Contribution (\alpha=0.1, \eta_g sweep)');
...
%% ========================================================================
fprintf('[Fig 6] Summary Table (alpha=0.1)\n');
figure('Position',[100 100 900 420]); axis off;

headers = {'Metric','\eta_g=0.1 (90%)','\eta_g=0.3 (70%)','\eta_g=0.5 (50%)'};
rows = {
    'Best Accuracy (%)',    '70.12',   '76.47',   '77.44'
    'Final Accuracy (%)',   '67.68',   '70.99',   '70.97'
    'Late-stage Mean (%)',  '68.00',   '73.80',   '73.91'
    'Late-stage Std (%)',   '1.33',    '1.52',    '2.17'
    'Time to 70% (h)',      '165.7',   '94.3',    '84.9'
...
for i = 1:nr
    ...
    for j = 1:nc
        ...
        % Lowest std → η_g=0.1
        if i==4 && j==2, bg=[0.85 0.93 0.85]; fw='bold'; end
...
title('FedPDA \eta_g Sweep Summary — \alpha=0.1 (Severe Non-IID)','FontSize',13,'FontWeight','bold');
exportgraphics(gcf, fullfile(FIG_DIR,'fig6_summary_table.png'), 'Resolution',300);
fprintf('  OK\n');

%% ========================================================================
fprintf('\n==============================\n');
fprintf(' 6개 그래프 생성 완료 (alpha=0.1 eta_g sweep)\n');
fprintf(' 저장 위치: %s/\n', FIG_DIR);
fprintf('==============================\n');
